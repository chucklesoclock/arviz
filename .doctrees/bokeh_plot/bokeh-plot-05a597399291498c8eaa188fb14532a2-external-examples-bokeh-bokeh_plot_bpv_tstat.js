(function() {
  var fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      var force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      var element = document.getElementById("7585c431-d65d-461e-819e-320756cb58eb");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid '7585c431-d65d-461e-819e-320756cb58eb' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error() {
          console.error("failed to load " + url);
        }
    
        for (var i = 0; i < css_urls.length; i++) {
          var url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error;
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        const hashes = {"https://cdn.bokeh.org/bokeh/release/bokeh-2.2.1.min.js": "qkRvDQVAIfzsJo40iRBbxt6sttt0hv4lh74DG7OK4MCHv4C5oohXYoHUM5W11uqS", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.2.1.min.js": "Sb7Mr06a9TNlet/GEBeKaf5xH3eb6AlCzwjtU82wNPyDrnfoiVl26qnvlKjmcAd+", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.2.1.min.js": "HaJ15vgfmcfRtB4c4YBOI4f1MUujukqInOWVqZJZZGK7Q+ivud0OKGSTn/Vm2iso"};
    
        for (var i = 0; i < js_urls.length; i++) {
          var url = js_urls[i];
          var element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error;
          element.async = false;
          element.src = url;
          if (url in hashes) {
            element.crossOrigin = "anonymous";
            element.integrity = "sha384-" + hashes[url];
          }
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      var js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.2.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.2.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.2.1.min.js"];
      var css_urls = [];
      
    
      var inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            var fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  var docs_json = '{"dc4e6394-6488-45f8-85af-4cf8c7bb1cab":{"roots":{"references":[{"attributes":{},"id":"69406","type":"BasicTicker"},{"attributes":{"toolbar":{"id":"69474"},"toolbar_location":"above"},"id":"69475","type":"ToolbarBox"},{"attributes":{"overlay":{"id":"69418"}},"id":"69413","type":"LassoSelectTool"},{"attributes":{},"id":"69395","type":"DataRange1d"},{"attributes":{"label":{"value":"bpv=0.53"},"renderers":[{"id":"69436"}]},"id":"69451","type":"LegendItem"},{"attributes":{"source":{"id":"69433"}},"id":"69437","type":"CDSView"},{"attributes":{"data_source":{"id":"69428"},"glyph":{"id":"69429"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"69430"},"selection_glyph":null,"view":{"id":"69432"}},"id":"69431","type":"GlyphRenderer"},{"attributes":{},"id":"69447","type":"Selection"},{"attributes":{},"id":"69412","type":"WheelZoomTool"},{"attributes":{},"id":"69397","type":"LinearScale"},{"attributes":{"line_alpha":0,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"69434","type":"Line"},{"attributes":{"items":[{"id":"69451"}]},"id":"69450","type":"Legend"},{"attributes":{},"id":"69414","type":"UndoTool"},{"attributes":{},"id":"69393","type":"DataRange1d"},{"attributes":{},"id":"69409","type":"ResetTool"},{"attributes":{"active_drag":"auto","active_inspect":"auto","active_multi":null,"active_scroll":"auto","active_tap":"auto","tools":[{"id":"69409"},{"id":"69410"},{"id":"69411"},{"id":"69412"},{"id":"69413"},{"id":"69414"},{"id":"69415"},{"id":"69416"}]},"id":"69419","type":"Toolbar"},{"attributes":{"formatter":{"id":"69442"},"ticker":{"id":"69402"}},"id":"69401","type":"LinearAxis"},{"attributes":{"fill_color":{"value":"#2a2eec"},"size":{"units":"screen","value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"69453","type":"Circle"},{"attributes":{"data_source":{"id":"69433"},"glyph":{"id":"69434"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"69435"},"selection_glyph":null,"view":{"id":"69437"}},"id":"69436","type":"GlyphRenderer"},{"attributes":{"toolbars":[{"id":"69419"}],"tools":[{"id":"69409"},{"id":"69410"},{"id":"69411"},{"id":"69412"},{"id":"69413"},{"id":"69414"},{"id":"69415"},{"id":"69416"}]},"id":"69474","type":"ProxyToolbar"},{"attributes":{},"id":"69470","type":"UnionRenderers"},{"attributes":{"axis":{"id":"69401"},"ticker":null},"id":"69404","type":"Grid"},{"attributes":{},"id":"69448","type":"UnionRenderers"},{"attributes":{"children":[{"id":"69475"},{"id":"69473"}]},"id":"69476","type":"Column"},{"attributes":{"data":{},"selected":{"id":"69449"},"selection_policy":{"id":"69448"}},"id":"69433","type":"ColumnDataSource"},{"attributes":{},"id":"69399","type":"LinearScale"},{"attributes":{"data_source":{"id":"69452"},"glyph":{"id":"69453"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"69454"},"selection_glyph":null,"view":{"id":"69456"}},"id":"69455","type":"GlyphRenderer"},{"attributes":{"fill_alpha":0.5,"fill_color":"lightgrey","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"xs_units":"screen","ys_units":"screen"},"id":"69418","type":"PolyAnnotation"},{"attributes":{},"id":"69449","type":"Selection"},{"attributes":{},"id":"69410","type":"PanTool"},{"attributes":{"source":{"id":"69428"}},"id":"69432","type":"CDSView"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"69429","type":"Line"},{"attributes":{},"id":"69440","type":"BasicTickFormatter"},{"attributes":{"data":{},"selected":{"id":"69471"},"selection_policy":{"id":"69470"}},"id":"69452","type":"ColumnDataSource"},{"attributes":{},"id":"69471","type":"Selection"},{"attributes":{"callback":null},"id":"69416","type":"HoverTool"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"69447"},"selection_policy":{"id":"69446"}},"id":"69428","type":"ColumnDataSource"},{"attributes":{"source":{"id":"69452"}},"id":"69456","type":"CDSView"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#2a2eec"},"line_alpha":{"value":0.1},"size":{"units":"screen","value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"69454","type":"Circle"},{"attributes":{},"id":"69415","type":"SaveTool"},{"attributes":{"text":"y","text_font_size":{"value":"15pt"}},"id":"69457","type":"Title"},{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"69430","type":"Line"},{"attributes":{"axis":{"id":"69405"},"dimension":1,"ticker":null},"id":"69408","type":"Grid"},{"attributes":{},"id":"69446","type":"UnionRenderers"},{"attributes":{"below":[{"id":"69401"}],"center":[{"id":"69404"},{"id":"69408"},{"id":"69450"}],"left":[{"id":"69405"}],"output_backend":"webgl","plot_height":500,"plot_width":500,"renderers":[{"id":"69431"},{"id":"69436"},{"id":"69455"}],"title":{"id":"69457"},"toolbar":{"id":"69419"},"toolbar_location":null,"x_range":{"id":"69393"},"x_scale":{"id":"69397"},"y_range":{"id":"69395"},"y_scale":{"id":"69399"}},"id":"69392","subtype":"Figure","type":"Plot"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"69435","type":"Line"},{"attributes":{},"id":"69442","type":"BasicTickFormatter"},{"attributes":{},"id":"69402","type":"BasicTicker"},{"attributes":{"overlay":{"id":"69417"}},"id":"69411","type":"BoxZoomTool"},{"attributes":{"children":[[{"id":"69392"},0,0]]},"id":"69473","type":"GridBox"},{"attributes":{"formatter":{"id":"69440"},"ticker":{"id":"69406"}},"id":"69405","type":"LinearAxis"},{"attributes":{"bottom_units":"screen","fill_alpha":0.5,"fill_color":"lightgrey","left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","top_units":"screen"},"id":"69417","type":"BoxAnnotation"}],"root_ids":["69476"]},"title":"Bokeh Application","version":"2.2.1"}}';
                  var render_items = [{"docid":"dc4e6394-6488-45f8-85af-4cf8c7bb1cab","root_ids":["69476"],"roots":{"69476":"7585c431-d65d-461e-819e-320756cb58eb"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    var attempts = 0;
                    var timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (var i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();